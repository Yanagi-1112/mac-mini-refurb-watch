...
- **USキーボード判定は詳細ページ方式に変更済み（2026-07-20）**: 実ページ検証の結果、一覧タイルのJSONにはキーボード情報が一切含まれないことが判明（Airタイル31件中0件）。キーボード表記は商品詳細ページにのみ存在する（JIS版は「JIS配列準拠キーボード」）。このため、Airの未知partNumberのみ詳細ページを取得して判定し、結果を `state.json` の `kb` にキャッシュする方式に変更した。US版の実際の表記は依然未確認（出品が稀）のため、出品を見つけたのに通知が来なかった場合は詳細ページの表記を確認して `US_KEYBOARD_RE` を修正すること。
